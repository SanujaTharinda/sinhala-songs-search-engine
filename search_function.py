    ...
    if (search_query.strip() != ""):
        ...
        if (year==0):
            ...
            if(len(search_fields)==0):
                query_es = search_quaries.multi_match_agg_best(processed_query, all_fields)
            ...
            if(len(year_array) > 1):
                ...
                if (len(processed_query) == 0):
                    query_es = search_quaries.multi_match_agg_year_without_query(years, all_fields)
                else:
                    query_es = search_quaries.multi_match_agg_year_with_query(processed_query, years, all_fields)
            else:
                if (len(processed_query) == 0):
                    query_es = search_quaries.multi_match_agg_single_year_without_query(year, ["year"])
                else:
                    query_es = search_quaries.multi_match_agg_single_year_with_query(processed_query, year, all_fields)
